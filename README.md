...
- `GET /api/queues` est� implementado y devuelve un `QueueSummary` normalizado.
- La UI ya muestra filtros locales, estados visuales, selecci�n de cola y panel lateral de detalle.
...
Es la pantalla principal del producto. Est� orientada a control operativo en tiempo real y combina:
...
Es la vista de gesti�n y contenido. Su objetivo es inspeccionar mensajes y operar sobre colas de forma directa. Incluye:
...
- `bun run build` todav�a tiene un bloqueo externo relacionado con Astro en este entorno Windows + Bun.
